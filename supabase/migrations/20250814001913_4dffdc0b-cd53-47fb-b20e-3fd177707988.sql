...
-- Create security definer function for user role check to avoid RLS recursion
...
CREATE POLICY "Users can view messages in their support tickets" ON public.support_messages
  FOR SELECT USING (
    ticket_id IN (
      ...
      WHERE user_id = auth.uid() OR public.get_user_role(auth.uid()) = 'admin'
    )
  );
...
CREATE POLICY "Users can upload chat files to their conversations" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'chat-files' AND
    auth.uid() = owner AND
    (storage.foldername(name))[1] IN (
