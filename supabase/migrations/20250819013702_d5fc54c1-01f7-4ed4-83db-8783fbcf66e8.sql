...
-- 2. Prevent users from changing their own role (privilege escalation fix)
...
-- 4. Secure order notifications - only service role should create them
...
-- 5. Secure payment splits - only service role should create them  
...
-- 6. Add policy to prevent unauthorized order status updates
CREATE POLICY "Only sellers and buyers can update order status" 
ON public.orders 
FOR UPDATE 
USING (
...
-- 7. Secure digital product files - create secure access function
