-- Fix critical security issues (handle existing policies)
...
DROP POLICY IF EXISTS "Service role can create order notifications" ON public.order_notifications;

CREATE POLICY "Service role creates order notifications" 
...
DROP POLICY IF EXISTS "Service role can create payment splits" ON public.payment_splits;

CREATE POLICY "Service role creates payment splits" 
ON public.payment_splits 
FOR INSERT 
WITH CHECK (auth.role() = 'service_role');

-- 5. Add policy to prevent unauthorized order status updates
DROP POLICY IF EXISTS "Only sellers buyers and service can update orders" ON public.orders;

CREATE POLICY "Secure order updates only" 
ON public.orders 
FOR UPDATE 
USING (
